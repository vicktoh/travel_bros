...
export const AvailableVehicles: FC<AvailableVehiclesProps> = ({ vehicles }) => {
  ...
  const fetchAvaliableVehilce = useCallback(async () => {
    try {
      ...
      const bookings = await getAvailableVehicles(
        ...
        vehicles,
        Number(params.get("numberOfSeats")),
      );
      setBookings(bookings || []);
      if (params.get("returnTrip") && params.get("returnDate")) {
        const returnBookings = await getAvailableVehicles(
          {
            to: params.get("from")!,
            from: params.get("to")!,
            date: params.get("returnDate")!,
          },
          vehicles,
          Number(params.get("numberOfSeats")),
        );
  ...
  const renderPageHeader = () => {
    switch (true) {
      case page === "select-vehicle" || page === "select-return-vehicle":
        return (
          <div className="flex flex-col max-w-full items-start justify-center relative z-10 md:px-0 px-5 mt-5">
            ...
            <p className="text-xl text-slate-300">{`Available ${
              page === "select-return-seats" ? "return " : ""
            } trips on ${params.get(
      ...
      case page === "Payment":
        return (
          <h3 className="text-3xl text-white relative z-10 justify-self-center self-center">
            Make Payment
          </h3>
        );
      case page === "success":
        return (
          <h3 className="text-3xl font-bold relative z-10 text-white justify-self-center self-center">
            Booking Successful
          </h3>
        );
      default:
